...
1. Bridge — a punctuation-only segment (e.g. a lone "—") sandwiched between
   ...
   surrounding field (e.g. an institution name split across a dash).
...
def _bridge_punctuation_gaps(
    ...
    result = list(macros)
    for i in range(1, len(education_segments) - 1):
        seg_tokens = education_segments[i].get("tokens") or []
        if not _is_punctuation_only(seg_tokens):
            continue
        prev_macro, next_macro = result[i - 1], result[i + 1]
        if (
            prev_macro
            and prev_macro == next_macro
            and prev_macro != result[i]
            and prev_macro in MACRO_TO_BIO_PREFIX
        ):
            result[i] = prev_macro
    return result
...
def apply_bio_stamps(
    ...
    macros = _bridge_punctuation_gaps(education_segments, resolved_labels)

    final_labels: dict[int, str] = {}
    prev_macro: str | None = None
    for seg, macro in zip(education_segments, macros):
