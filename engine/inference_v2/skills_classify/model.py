"""SkillsSegmentClassifierModel — mirrors training_pipeline/skills/model.py."""
...
class SkillsSegmentClassifierModel(nn.Module):
    def __init__(
        self,
    ...
    def forward(
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            token_embeddings = torch.zeros(
                (batch_size * num_segs, seg_len, text_dim),
            ...
            if len(non_pad_indices) > 0:
                ...
                token_embeddings[non_pad_indices] = enc_out.last_hidden_state.to(token_embeddings.dtype)

        text_embeddings = token_embeddings.view(batch_size, num_segs, seg_len, text_dim)
        ...
        spatial_expanded = spatial_embeddings.unsqueeze(2).expand(-1, -1, seg_len, -1)
        fused = self.dropout(self.glu_fusion(text_embeddings, spatial_expanded))
