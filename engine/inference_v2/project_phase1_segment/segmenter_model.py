"""Phrase segmenter architecture for project phase-1 token segmentation.

Mirrors ``training_pipeline/project/phase1_token_segmentation/src/model.py`` so the
exported ``best_model.pt`` loads with fully-matching weights: distilled encoder →
...
spatial-bias, no text_projection) and is incompatible with this checkpoint.
"""
