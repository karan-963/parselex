"""Expand B-PROJ_START head lines into full entry spans (B/I-PROJ_START until next entry)."""
...
def expand_project_span_labels(tokens: list[dict], word_preds: list[str]) -> list[str]:
    """Mark all tokens between entry heads as I-PROJ_START; first alnum on each head line = B-PROJ_START."""
    ...
    b_proj_lines: list[tuple[int, int]] = []
    for line_key in ordered_lines:
        indices = sorted(by_line[line_key], key=lambda i: tokens[i].get("tokenIndex", i))
        if any(word_preds[i] == "B-PROJ_START" for i in indices):
            b_proj_lines.append(line_key)
    ...
    for bi, bl in enumerate(b_proj_lines):
        start = line_pos[bl]
        end = line_pos[b_proj_lines[bi + 1]] if bi + 1 < len(b_proj_lines) else len(ordered_lines)
