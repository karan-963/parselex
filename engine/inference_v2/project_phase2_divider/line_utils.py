"""Physical-line helpers for project phase-2 boundary eval."""
...
def build_physical_line_text_map(
    ...
    for seg in segments:
        for t in seg.get("tokens", []):
            if t and "page" in t and "lineIndex" in t:
                coords.add((p := t["page"], l := t["lineIndex"]))
...
def group_has_boundary_start(seg_preds: list[str], group: list[int]) -> bool:
    return any(seg_preds[idx] == "B-PROJ_START" for idx in group)
...
def collapse_boundary_to_line_anchor(
    ...
    """One B-PROJ_START per line group; anchor = first in-section segment (not punct-only prefix)."""
    out = list(seg_preds)
    for group in groups:
        ...
        for idx in group:
            out[idx] = "O"
        anchor = next(
            (i for i in group if is_project_segment(segments[i])),
            group[0],
        )
        out[anchor] = "B-PROJ_START"
    return out
...
def assign_line_level_token_predictions(
    ...
    """First token of every segment on a predicted line gets B-PROJ_START (whole-line mark)."""
    ...
    for idx in range(num_segs):
        ...
        label = "B-PROJ_START" if line_pred else seg_preds[idx]
